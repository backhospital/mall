...
public class AdminController {
	...
	public String postGoodsRegister(GoodsVO vo, MultipartFile file) throws Exception {
		...
		String imgUploadPath = uploadPath + File.separator + "images";  // 이미지를 업로드할 폴더를 설정 = /uploadPath/images 
		...
		if(file.getOriginalFilename() != null && file.getOriginalFilename() != "") {
			...
			vo.setGdsImg(File.separator + "images" + File.separator + fileName);
			// gdsThumbImg에 썸네일 파일 경로 + 썸네일 파일명 저장
			vo.setGdsThumbImg(File.separator + "images" + File.separator + "s" + File.separator + "s_" + fileName);
	...
	// 주문목록2
	@RequestMapping(value = "/shop/orderList2", method = RequestMethod.GET)
	public void getOrderList2(Model model) throws Exception {
		logger.info("get order list2");
		
		List<OrderVO> orderList2 = adminService.orderList2();
	...
	// 회원 목록
	@RequestMapping(value = "/goods/memberlist", method = RequestMethod.GET)
	...
	// 회원정보 조회
	@RequestMapping(value = "goods/memberview", method = RequestMethod.GET)
	public void getMemberView(@RequestParam("useridx") int useridx, Model model) throws Exception {
		
		MemberVO vo = memberService.memberview(useridx);
		model.addAttribute("memberview", vo);
	}
	
	// 회원정보 수정 : get
	@RequestMapping(value = "goods/membermodify", method = RequestMethod.GET)
	...
	// 회원정보 수정 : post
	@RequestMapping(value = "goods/membermodify", method = RequestMethod.POST)
	public String postMemberModify(MemberVO vo) throws Exception {
		logger.info("post member modify");
		
		memberService.memberModify(vo);
		return "redirect:/admin/goods/memberlist";
	}

	// 회원정보 삭제 
	@RequestMapping(value = "goods/memberdelete")
